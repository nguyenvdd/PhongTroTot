...
import { Helmet } from "react-helmet" // ✅ Thêm Helmet
...
const DetailPost = () => {
  ...
  return (
    <>
      {/* ✅ Helmet để SEO Facebook */}
      {post && (
        <Helmet>
          <title>{post?.title}</title>
          <meta property="og:title" content={post?.title} />
          <meta property="og:description" content={post?.address || "Tin đăng hấp dẫn"} />
      ...
      <div className="w-full p-4 lg:w-main mx-auto grid grid-cols-7 lg:grid-cols-10 gap-4">
        <div className="col-span-7 my-4 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            ...
            <span>
              Ngày cập nhật gần nhất: <span>{moment(post?.updatedAt).format("DD/MM/YYYY")}</span>
            </span>
          </div>
