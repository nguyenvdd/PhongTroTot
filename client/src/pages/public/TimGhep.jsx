...
import ListTimOGhep from "~/components/posts/ListTimOGhep"
...
const TimGhep = () => {
  ...
  const handleFilter = (data) => {
    ...
    if (data.priceStart) searchParamsObj["gia-tu"] = data.priceStart
    if (data.priceEnd) searchParamsObj["gia-den"] = data.priceEnd
    ...
    <main className="w-full bg-white lg:w-main px-4 py-4 mx-auto">
      ...
      <div className="w-full grid grid-cols-10 mt-4 gap-4">
        ...
        <form className="col-span-10 flex flex-col gap-4 md:col-span-3">
          ...
          <BoxFilter className="flex flex-col gap-4" title="Giá thuê">
            <InputForm
              register={register}
              id="priceStart"
              errors={errors}
