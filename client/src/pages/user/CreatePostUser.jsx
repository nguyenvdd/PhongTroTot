...
const CreatePostUser = () => {
  ...
  const [createRoomMode, setCreateRoomMode] = useState("MANUAL")
  ...
  const handleAddRoom = (room) => {
    if (rooms.length >= 1) {
      toast.warning("Chỉ có thể thêm 1 phòng duy nhất.")
    } else if (rooms.some((el) => el.title === room.title)) {
  ...
  return (
    <div className="w-full h-full">
      ...
      <form className="p-4 w-full">
        <h2 className="font-bold text-[#007370] my-4">1. Thông tin tổng quát</h2>
        <div className="flex flex-col gap-4">
          ...
          <InputSelect
            ...
            validate={{ required: "Không được bỏ trống." }}
            options={catalogs
              ?.filter((el) => el.value === "Tìm người ở ghép")
              .map((el) => ({
                ...el,
                label: el.value,
                value: el.id,
          ...
          <InputFile
            ...
            mutilple={true}
          />
          <h2 className="font-bold text-[#007370] my-4">2. Chi tiết phòng ở</h2>
                ...
                <tbody>
                  {rooms.map((el, idx) => (
                    <tr key={idx}>
                      ...
                      <td className="text-center border text-[#007370] p-3">
