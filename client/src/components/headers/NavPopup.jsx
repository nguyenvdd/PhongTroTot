...
const NavPopup = ({ setIsShowMenu }) => {
  ...
  return (
    ...
    >
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              clsx(
                ...
                isActive && "text-[#007370] bg-gray-100"
              )
            }
            to={`/${el.slug}`}
          >
          ...
          (el) =>
            current?.rroles?.some((n) => n.roleCode === el.code) && (
              <Link
                key={el.id}
                to={el.path}
                className="py-3 bg-transparent border-[#007370] border flex justify-center rounded-md text-[#007370]"
              >
                {el.name}
              </Link>
            )
      ...
      <div className="p-4 bg-[#007370] text-white flex items-center justify-between">
        {current ? (
