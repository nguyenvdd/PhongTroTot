...
module.exports = {
  ...
  getDashboardManager: asyncHandler(async (req, res) => {
    ...
    const [postData, post, classifyPost, views, roomStatus] = await Promise.all([
      db.Post.findAll({
        where: q,
        attributes: [
    ...
    return res.json({
      success: true,
      data: { postData, post, classifyPost, views, roomStatus },
    })
  }),
  getDashboardAdmin: asyncHandler(async (req, res) => {
    ...
    const [postData, posts, views, userCount, userRoles, paymentStatus] = await Promise.all([
      db.Post.findAll({
        where: q,
        attributes: [
          [Sequelize.fn("to_char", Sequelize.col("createdAt"), typeDate), "createdOn"],
    ...
    return res.json({
      success: true,
      data: { postData, posts, views, paymentStatus, userRoles, userCount },
    })
  }),
  getDashboardAdminManager: asyncHandler(async (req, res) => {
    const { days, type, from, to, id } = req.query
    const daysQuery = days || 220
    const typeDate = type === "MONTH" ? "MM-YY" : "DD-MM-YY"
    const start = from || Date.now() - daysQuery * 24 * 60 * 60 * 1000
    const end = to || Date.now()
  ...
  getDashboardRoom: asyncHandler(async (req, res) => {
    const { days, type, from, to, idUser, idRoom } = req.query
    const daysQuery = days || 220
    const typeDate = type === "MONTH" ? "MM-YY" : "DD-MM-YY"
    const start = from || Date.now() - daysQuery * 24 * 60 * 60 * 1000
    const end = to || Date.now()
    const q = { postedBy: idUser }
    if (from && to) {
      if (from === to)
        q.createdAt = {
          [Op.and]: [{ [Op.gte]: `${from} 00:00:00` }, { [Op.lte]: `${from} 23:59:59` }],
